...
export default class extends Command {
  constructor(client: Bot) {
    super(client, {
      ...
      args: [
        new Arg("member", "The member to remove xp from", true),
        new Arg("amount", "The amount of xp to remove", true),
      ],
      devOnly: false,
  ...
  public async run(msg: Message, { args }: CommandRunOptions): Promise<CommandResult | Message> {
    ...
    await msg.send("success", `Removed **${amt}** xp from ${target.user.tag}`);
    return { done: true };
  }
}
